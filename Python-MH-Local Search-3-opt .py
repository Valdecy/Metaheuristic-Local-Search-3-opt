...
X = pd.read_csv('Python-MH-Local Search-3-opt-Dataset-01.txt', sep = '\t') # 17 cities = 1922.33
seed = seed_function(X)
ls3opt = local_search_3_opt(X, city_tour = seed, recursive_seeding = 10)
plot_tour_distance_matrix(X, ls3opt) # Red Point = Initial city; Orange Point = Second City # The generated coordinates (2D projection) are aproximated, depending on the data, the optimum tour may present crosses.

Y = pd.read_csv('Python-MH-Local Search-3-opt-Dataset-02.txt', sep = '\t') # Berlin 52 = 7544.37
X = buid_distance_matrix(Y)
seed = seed_function(X)
ls3opt = local_search_3_opt(X, city_tour = seed, recursive_seeding = 10)
plot_tour_coordinates (Y, ls3opt) # Red Point = Initial city; Orange Point = Second City
